...
function openJackpotTicket() {
  ...
  openModal(`
    <div class="jackpot-ticket-modal memorial-ticket-modal">
      <div class="analysis-modal-head ticket-modal-head">
        <div>
          <p class="eyebrow">MEMORIAL BET TICKET</p>
          <h2>一撃王馬券</h2>
      ...
      <section class="kyoto-memorial-ticket" aria-label="一撃王記念馬券">
        <aside class="km-side">
          <div class="km-side-title">
            <small>HORSE BET BATTLE</small>
            <strong>MEMORIAL<br>BET TICKET</strong>
            <span>MAX PAYOUT RECORD</span>
          </div>

          <div class="km-side-info">
            <span>${esc(fullTicketDate(race.raceDateTime))}</span>
            <strong>${esc(venue)}</strong>
            <b>${esc(raceNo)}</b>
            <small>${esc(grade)}</small>
            <em>${esc(race.name)}</em>
          </div>

          <svg class="km-side-art" viewBox="0 0 220 120" aria-hidden="true">
            <g fill="none" stroke="currentColor" stroke-width="1.35">
              <path d="M10 96 H205"/>
              <path d="M22 86 H192 M36 75 H180 M48 64 H168"/>
              <path d="M24 96 V58 L57 45 L105 58 V96 M57 45 V96"/>
              <path d="M105 58 L162 43 L198 58 V96"/>
              <path d="M18 102 C55 87 86 87 112 102 C144 87 174 87 207 102"/>
              <path d="M149 84 q10-16 20 0 q10-16 20 0"/>
            </g>
          </svg>
        </aside>

        <main class="km-main">
          <div class="km-main-brand">
            <small>HORSE BET BATTLE / DIGITAL MEMORIAL</small>
            <strong>一撃王馬券</strong>
          </div>

          <div class="km-top-grid">
            <div class="km-player-cell">
              <small>PLAYER</small>
              <div class="km-player-row">
                ${silkMark(competition, participant, 'jockey-silk km-silk')}
                <strong>${esc(participant.name)}</strong>
              </div>
            </div>

            <div class="km-race-cell">
              <small>RACE</small>
              <strong>${esc(raceNo)} ${esc(race.name)}</strong>
              <span>${esc(venue)}　${esc(grade)}${race.distance ? `　${esc(race.distance)}m` : ''}</span>
            </div>
          </div>

          <div class="km-data-grid">
            <div>
              <small>DATE</small>
              <strong>${esc(fullTicketDate(race.raceDateTime))}</strong>
            </div>
            <div>
              <small>BET</small>
              <strong>¥${bet.toLocaleString('ja-JP')}</strong>
            </div>
            <div class="km-payout">
              <small>PAYOUT</small>
            ...
            <div>
              <small>RETURN</small>
              <strong>${pct(recovery)}</strong>
            </div>
          </div>

          <div class="km-course-art" aria-hidden="true">
            <svg viewBox="0 0 760 180">
              <g fill="none" stroke="currentColor">
                <path d="M20 146 C135 110 250 111 365 146 C476 178 602 173 735 128" stroke-width="2"/>
                <path d="M26 158 C143 123 257 124 368 157 C478 188 607 184 742 140" stroke-width="1"/>
                <path d="M82 112 H692 M110 98 H655 M150 84 H615" stroke-width="1"/>
                <path d="M112 112 V65 L176 50 L263 65 V112 M176 50 V112"/>
                <path d="M263 65 L365 45 L432 65 V112"/>
                <path d="M480 112 V62 L540 47 L628 62 V112 M540 47 V112"/>
                <path d="M628 62 L692 53 L721 64 V112"/>
                <path d="M315 126 q22-32 44 0 q22-32 44 0" stroke-width="1.5"/>
                <path d="M395 126 q15-23 30 0" stroke-width="1.2"/>
              </g>
            </svg>
          </div>

          <div class="km-barcode"></div>

          <div class="km-foot">
            <span>${esc(ticketId)}</span>
            <b>${esc(competition.name)}</b>
          </div>
        </main>

        <div class="km-vertical-band">
          <span>HORSE BET BATTLE</span>
          <b>MEMORIAL</b>
        </div>
      </section>

      <div class="km-ticket-note">
        <span>一撃王馬券は現在の最大払戻記録を表示しています。</span>
      </div>
    </div>`);
}
