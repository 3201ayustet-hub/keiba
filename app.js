...
function openJackpotTicket() {
  ...
  openModal(`
    <div class="jackpot-ticket-modal horizontal-ticket-modal">
      ...
      <section class="horizontal-bet-ticket" aria-label="一撃王馬券">
        <div class="hbt-left">
          <div class="hbt-left-top">
            <span>HORSE BET BATTLE</span>
            <strong>${esc(competition.name)}</strong>
          </div>

          <div class="hbt-race-number">${esc(raceNo)}</div>

          <div class="hbt-venue">
            <small>${esc(fullTicketDate(race.raceDateTime))}</small>
            <strong>${esc(venue)}</strong>
            <span>${esc(grade)}</span>
          </div>

          <div class="hbt-left-race">
            <small>RACE</small>
            <strong>${esc(race.name)}</strong>
          </div>

          <div class="hbt-left-id">
        ...
        <div class="hbt-main">
          ...
          <div class="hbt-race-line">
            <div>
              <small>RACE / ${esc(grade)}</small>
              <strong>${esc(raceNo)}　${esc(race.name)}</strong>
              <span>${esc(venue)}${race.distance ? `　${esc(race.distance)}m` : ''}</span>
            </div>
            <b>${esc(grade)}</b>
          </div>

          <div class="hbt-player-money">
            <div class="hbt-player">
              ${silkMark(competition, participant, 'jockey-silk hbt-silk')}
              <div>
                <small>PLAYER</small>
            ...
            <div class="hbt-payout">
              <small>PAYOUT</small>
              <strong>¥${payout.toLocaleString('ja-JP')}</strong>
            </div>
          </div>

          <div class="hbt-bottom-data">
            <div>
              <small>BET</small>
          ...
          <div class="hbt-sketch" aria-hidden="true">
            <svg viewBox="0 0 960 180">
              <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round">
                <path d="M20 142 C180 104 350 100 495 136 C635 171 785 164 935 120" stroke-width="1.8"/>
                <path d="M25 153 C185 117 352 113 500 149 C642 182 790 176 940 132" stroke-width="1"/>
                <path d="M85 108 H360 M105 95 H345 M130 82 H325" stroke-width="1"/>
                <path d="M105 108 V68 L165 52 L230 68 V108 M165 52 V108 M230 68 L300 55 L350 68 V108" stroke-width="1.2"/>
                <path d="M650 108 V64 L710 48 L785 64 V108 M710 48 V108 M785 64 L845 54 L890 68 V108" stroke-width="1.2"/>
                <path d="M515 104 q15-28 30 0 q15-28 30 0 q15-28 30 0" stroke-width="1.4"/>
                <path d="M734 98 V132 M734 98 H760 M760 98 V122" stroke-width="1.8"/>
                <path d="M718 122 H778 M718 130 H778" stroke-width="1"/>
                <path d="M430 112 q7-10 15 0 q8-10 16 0" stroke-width=".8"/>
                <path d="M465 106 q8-12 17 0 q8-12 17 0" stroke-width=".8"/>
              </g>
            </svg>
          </div>

          <div class="hbt-barcode-row">
            <div class="hbt-barcode"></div>
            <div class="hbt-serial">
              <span>${esc(ticketId)}</span>
              <b>MAX PAYOUT HOLDER</b>
            </div>
          </div>
        </div>
      ...
      <div class="hbt-note">一撃王馬券は現在の最大払戻記録を表示しています。</div>
    </div>`);
}
