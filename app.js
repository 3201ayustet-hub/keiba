...
function validConfig() {
  return window.APP_CONFIG
    && /^https:\/\/.+\.supabase\.co$/.test(window.APP_CONFIG.supabaseUrl || '')
    && /^(sb_publishable_|eyJ)/.test(window.APP_CONFIG.supabasePublishableKey || '');
}
...
async function loadCloudState() {
  ...
  const cloudState = data?.data && Array.isArray(data.data.competitions) ? data.data : { competitions: [] };
  const savedLocal = localState();
  if (!cloudState.competitions.length && savedLocal.competitions.length) {
    const migrate = confirm('この端末に保存されている既存データをSupabaseへ移行しますか？');
    if (migrate) {
      state = savedLocal;
      await persistState();
      return;
...
async function persistState(snapshot = state) {
...
function subscribeRealtime() {
  supabaseClient.channel('horse-bet-battle-sync')
    .on('postgres_changes', {
      ...
      if (incoming && Array.isArray(incoming.competitions)) {
        state = incoming;
        render();
        showToast('別の端末の変更を反映しました');
      }
    })
    .subscribe();
}
...
function showSetupError(message) {
  ...
  app.innerHTML = `<section class="section card setup-card">
    ...
    <p>${esc(message)}</p>
    <ol>
      <li>SupabaseでSQLを実行します。</li>
      <li><code>config.js</code>へProject URLとPublishable Keyを貼り付けます。</li>
      <li>GitHubへファイルをアップロードし直します。</li>
    </ol>
  </section>`;
}
...
async function startApp() {
  ...
  if (!validConfig()) return showSetupError('config.js のURLまたはPublishable Keyが未設定です。');
  if (!window.supabase?.createClient) return showSetupError('Supabaseライブラリを読み込めませんでした。通信環境を確認してください。');
  try {
    ...
    console.error(error);
    showSetupError(`Supabaseへの接続に失敗しました。SQL実行とconfig.jsを確認してください。（${error.message || '不明なエラー'}）`);
  }
}
