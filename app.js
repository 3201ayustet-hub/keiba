...
function openRankHistory() {
  ...
  const lines = participants.map((participant, pi) => {
    ...
    const dots = coords.map(([x,y,row], idx) => `<circle class="chart-point" tabindex="0" role="button" data-point-index="${idx}" data-participant-id="${esc(participant.id)}" cx="${x}" cy="${y}" r="4.5" fill="${palette[pi % palette.length]}"/>`).join('');
    return `<path d="${path}" fill="none" stroke="${palette[pi % palette.length]}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>${dots}`;
  }).join('');
  ...
  openModal(`<div class="detail-head chart-head"><div><p class="eyebrow gold-text">RANK HISTORY</p><h2>最大払戻額 順位推移</h2><p class="muted">直近5レースを表示。左へスワイプすると過去を確認できます。</p></div><button class="button ghost" type="button" data-close-modal>閉じる</button></div>
  ...
  function updateEndLabels() {
    ...
    endLabels.innerHTML = point.ranked.map(row => {
      ...
      return `<span style="top:${y}%;--label-color:${palette[pi % palette.length]}" title="${esc(row.name)}"><i></i>${esc(shortName)}</span>`;
    }).join('');
