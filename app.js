...
function renderCompetition(competition) {
  ...
  const races = [...competition.races].sort((a, b) => {
  ...
  app.innerHTML = `
    ...
    <section class="section">
      <div class="section-head compact action-head"><h2>登録レース</h2><button class="button primary compact-action" type="button" data-action="add-race" ${competition.participants.length ? '' : 'disabled'}>＋ レース</button></div>
    ...
    <section class="section">
      <div class="section-head compact action-head"><div><h2>参加者</h2><p class="muted">カードを選ぶと個人成績を確認できます。</p></div><button class="button primary compact-action" type="button" data-action="add-participant" ${competition.participants.length >= 8 ? 'disabled' : ''}>＋ 参加者</button></div>
    ...
    ${competition.topContent ? `<section class="section rules-section"><div class="section-head compact"><div><p class="eyebrow">RULES & PRIZE</p><h2>ルール・賞品</h2></div></div><div class="rules rules-bottom">${esc(competition.topContent)}</div></section>` : ''}
...
function raceListItem(competition, race) {
  ...
  return `<article class="race-list-item compact-race" data-race-id="${esc(race.id)}">
    <button class="race-name-button" type="button" data-action="view-race" aria-label="${esc(race.name)}の詳細を開く">
      <span class="badge race-grade">${race.gradeType === 'G1' ? 'G1' : 'G1以外'}</span>
      <span class="race-name-text">${esc(race.name)}</span>
      <span class="race-summary">入力 ${completed}/${competition.participants.length}人 ・ 参加 ${joined}人</span>
