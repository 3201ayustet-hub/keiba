...
function openJackpotTicket() {
  ...
  if (!champ) {
    openModal(`<div class="analysis-modal"><div class="analysis-modal-head"><div><p class="eyebrow">JACKPOT TICKET</p><h2>一撃王馬券</h2></div><button class="button ghost" type="button" data-close-modal>閉じる</button></div><div class="score-empty">払戻記録がありません。</div></div>`);
    return;
  }
  ...
  const ticketId = `HBB-${String(race.raceDateTime || '').slice(0,10).replace(/-/g,'')}-${String(race.id || '').slice(-6).toUpperCase()}`;

  openModal(`
    <div class="jackpot-ticket-modal">
      ...
      <section class="jackpot-ticket">
        <div class="ticket-brand">HORSE BET BATTLE / DIGITAL BET TICKET</div>

        <div class="jackpot-ticket-race">
          <div>
            <strong>${esc(race.name)}</strong>
            <span>${race.gradeType === 'G1' ? 'G1' : '非G1'}</span>
          </div>
          <b>${esc(raceDateDisplay(race.raceDateTime))}</b>
        </div>

        <div class="jackpot-player">
          ${silkMark(competition, participant, 'jockey-silk jackpot-silk')}
          <div>
            <small>PLAYER</small>
        ...
        <div class="jackpot-ticket-grid">
          <div><small>BET</small><strong>¥${bet.toLocaleString('ja-JP')}</strong></div>
          <div><small>PAYOUT</small><strong>¥${payout.toLocaleString('ja-JP')}</strong></div>
          <div><small>RETURN</small><strong>${pct(recovery)}</strong></div>
          <div><small>RACE</small><strong>${esc(compactRacecourse(race.racecourse)) || '—'}</strong></div>
        </div>

        <div class="jackpot-barcode" aria-hidden="true"></div>

        <div class="jackpot-ticket-foot">
